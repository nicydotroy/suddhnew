          ...
          <div className="container mx-auto px-4 max-w-3xl sm:px-6 lg:px-8">
            ...
            <div className="prose max-w-none mb-12">
              {article.content.split('\n\n').map((paragraph, index) => (
                <p key={index} className="mb-4 text-lg leading-8 text-gray-700">
                  {paragraph}
                </p>
              ))}
            </div>
              ...
              <div className="flex flex-wrap gap-2">
                {article.tags.map((tag) => (
                  <Link
                    key={tag}
                    href={`/tags/${tag}`}
                    className="inline-block rounded-full bg-gray-100 px-4 py-2 text-sm text-blue-600 hover:bg-gray-200"
                  >
                    #{tag}
                  </Link>
                ))}
